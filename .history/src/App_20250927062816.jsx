   ...
   <MobileNavMenu setOpened={setOpened} /> :
    <> {!isSocialPage && !isAgeVerificationPage && <Header setOpened={open} />} <div> 
      ...
      <Routes>
         ...
         <Route path='/orderStatus' element={<OrderStatus />}/> <Route path='/collaborators' element={<Collaborators />}/> <Route path='/share' element={<SocialShare />}/> <Route path='*' element={<Navigate to="/"/>}/> </Routes> </div> {!isSocialPage && !isAgeVerificationPage && <Footer />} </> } </div> </LanguageProvider> ); } export default App;