        ...
        <div className={styles.textContent}>
          ...
          <div className={styles.description}>
            {texts.description.map((item, index) => {
              ...
              const paragraph = typeof item === 'string' ? item : item.text;
              const isCentered = typeof item === 'object' ? item.centered : (index === 2 || index === 3);
              const shouldAddMargin = typeof item === 'string' ? item.trim() : item.text.trim();
              
              return (
                <p 
                  key={index} 
                  className={isCentered ? styles.centeredText : ""}
                  style={{ marginBottom: shouldAddMargin ? "1rem" : "0.5rem" }}
                >
                  {paragraph}
                </p>
              );
